...
using ModularDoc.Printer;
using ModularDoc.Printer.Markdown;
using ModularDoc.ViewModels.GitMarkdown;
using ModularDoc.Views.GitMarkdown;
...
namespace ModularDoc.Plugins.GitMarkdown
{
    ...
    : Module, IPlugin
  {
    ...
    public (IModularDocLogger logger, IReadOnlyCollection<IProcess> processes, Func<CancellationToken, ValueTask> executor) GenerateExecutor(IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> configuration)
    {
      ...
      var logger = TypeResolver.Resolve<IModularDocLogger>();

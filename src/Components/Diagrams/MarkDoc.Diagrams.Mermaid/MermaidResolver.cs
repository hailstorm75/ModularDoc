    ...
    : IDiagramResolver
  {
    ...
    public bool TryGenerateDiagram(IType type, out (string name, string content) diagram)
    {
      ...
      (string nameSpace, string typeDiagram) GenerateType(IType type, bool isParent = false)
      {
        ...
        {
          IRecord r => $"class {GenerateTitle(r)} << (R, orchid) >>",
      ...
      diagram = (type.RawName.Replace('`', '_'), $"classDiagram{Environment.NewLine}{string.Join(Environment.NewLine, PackTypes(types).Concat(relations))}");
      return true;
    }

    private static IEnumerable<string> PackTypes(IReadOnlyDictionary<string, LinkedList<string>> types)
      => types.Select(type => string.Join(Environment.NewLine, type.Value)).ToList();

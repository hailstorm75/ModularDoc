  ...
  : IDocElement
{
  ...
  private static readonly HashSet<TagType> SINGLE =
  [
    TagType.Summary,
    TagType.Value,
    TagType.Remarks,
  ...
  private static readonly HashSet<TagType> MULTIPLE =
  [
    TagType.Exception,
    TagType.Param,
    TagType.Typeparam,
  ...
  private static void ProcessInheritDoc(IReadOnlyDictionary<string, (string rawName, Dictionary<TagType, LinkedList<ITag>> tags)> cache, IInterface type, string[] names, IReadOnlyDictionary<string, IDocMember> memberDocs, IDocResolver docResolver)
  {
    ...
    var baseClass = type is IClass { BaseClass.Reference.Value: not null } classDef
      // return it

...
namespace MarkDoc.Members.Dnlib
{
    ...
    : IResolver
  {
    ...
    public void Resolve(string assembly)
    {
      ...
      settings.SetupProperty(x => x.IgnoredNamespaces, Array.Empty<string>());
      settings.SetupProperty(x => x.IgnoredTypes, Array.Empty<string>());

      Resolve(assembly, settings.Object);
    }
    ...
    private void Resolve(string assembly, IGlobalSettings globalSettings)
    {
      bool FilterNamespaces(IGrouping<string, TypeDef> grouping)
      {
        ...
        return !string.IsNullOrEmpty(typeNamespace)
               ...
               && !globalSettings.IgnoredNamespaces.Contains(processed);
      }
      ...
      var group = module
        ...
        .Where(type => !globalSettings.IgnoredTypes.Contains(type.FullName))

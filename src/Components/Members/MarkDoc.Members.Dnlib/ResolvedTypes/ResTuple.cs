...
namespace MarkDoc.Members.Dnlib.ResolvedTypes
{
    ...
    : ResType, IResTuple
  {
    ...
    internal ResTuple(Resolver resolver,
      ...
      bool isByRef = false)
      : base(resolver, source, ResolveName(resolver, source, isValueTuple, generics, dynamicsMap, tupleMap, out var fields), ResolveDocName(source), source.FullName, isByRef)
    {
      IsValueTuple = isValueTuple;
      Fields = fields;
    }

    private static string ResolveName(Resolver resolver, TypeSig source, bool isValueTuple, IReadOnlyDictionary<string, string>? generics, IReadOnlyList<bool>? dynamicsMap, IReadOnlyList<string>? tupleMap, out IReadOnlyCollection<(string, IResType)> fields)
    {
      ...
      static IReadOnlyList<string>? GetTupleNames(int i, IEnumerable<string>? map, IReadOnlyList<int> tree)
      {
        // If there are no dynamic types in the arguments..
        if (map is null)
          // return nothing
      ...
      fields = token.GenericArguments.Select((x, i) => (isValueTuple && tupleMap != null ? tupleMap[i] : $"Item{i + 1}", resolver.Resolve(x, generics, false, GetGenerics(i, dynamicsMap, genericType.CountTypes()), GetTupleNames(i, tupleMap, genericType.CountTypes(true))))).ToReadOnlyCollection();
      return isValueTuple
